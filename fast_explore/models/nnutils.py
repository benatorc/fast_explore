...
def get_bond_indices_from_batch(fmess, scope, edge_scope):
    out = []
    for idx, (st, _) in enumerate(scope):
        e_st, e_le = edge_scope[idx]
        # extract the bonds for the current mol, and subtract the node offset
        out.append(fmess[e_st:e_st+e_le][:, :2] - st)
    return stack_pad_tensor(out)
